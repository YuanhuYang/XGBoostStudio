...
- **CI 失败** 或 PR 需要多轮修复时，与 `.cursor/AGENTS.md` 中的看护流程一致执行。
...
5. **执行**：在 `server` 下跑 Python 测试；客户端按 `package.json` scripts；记录 **一条可复制** 的全量命令写入 `.cursor/AGENTS.md`。
