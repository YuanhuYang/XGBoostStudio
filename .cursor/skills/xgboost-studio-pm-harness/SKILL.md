...
### 多迭代示例：质量与商用交付（两阶段；迭代 ID 由 PM 分配）

当用户方向包含 **优化质量、全量测试、商用化交付**（或同义表述）时，**项目经理**可在路线图中 **默认给出至少两行**（除非用户明确要求单迭代）。下表 **不固定代号**，仅说明两束的语义；**迭代 ID**（如 `I1`、`I2` 或仓库既有规范）由 PM 在路线图中写明。

| 束序 | 目标（摘要） | 范围冻结要点 | C 阶段侧重点 |
|------|----------------|----------------|----------------|
| 第一束 | 质量基线：跑通章程规定的全量/子集命令，产出失败清单与复现 | 以执行与记录为主；代码改动仅限解除「无法跑测」的阻塞 | `xgboost-studio-test-delivery`：证据与清单 |
| 下一束 | 修复与交付：按上一束清单修缺陷，再全量回归直至 DoD | 仅清单内缺陷与契约；仍禁止新功能 | 同上 + 发版时对齐 `RELEASE_CHECKLIST` / 版本线 |

依赖：**后一束** 依赖 **前一束** 的冻结输出（或 PM 书面确认的等价基线）。详细启发式见 **`xgboost-studio-role-project-manager`** §多迭代规划。
