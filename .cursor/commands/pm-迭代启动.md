...
> **版本线（建议追问）**：本轮产出计划纳入哪一个**产品版本**（SemVer，如下一 MINOR `0.x.0`）还是**仅主干、不发版**？详见 [`docs/product/版本与发布.md`](../../docs/product/版本与发布.md)。可 `@xs-release-versioning`。
...
1. **《迭代路线图》**（可按需 **一次规划多行迭代**；每行：ID、目标、范围冻结、交付定义、依赖、测试子集、建议独立 Agent 会话；可选列 **目标产品版本** 或与 [`版本与发布`](../../docs/product/版本与发布.md) 表的关系）。  
