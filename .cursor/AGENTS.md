...
**迭代方向分流**（研发 / 质量 / 体验 / PM 流程 / 需求 / 运维 / 文档等 → 用哪个 Subagent）：见 **[`迭代方向与能力映射.md`](迭代方向与能力映射.md)**，Skill **`xgboost-studio-iteration-intents`**。
...
| **项目 Skill（项目经理）** | 排期、优先级、与五领域共评、防蔓延 | `.cursor/skills/xgboost-studio-role-project-manager/SKILL.md` |
| **项目 Skill（多领域评审）** | 汇总格式；迭代内仅审视冻结范围 | `.cursor/skills/xgboost-studio-multi-agent-review/SKILL.md` |
| **项目 Skill（五领域分角）** | 各领域独立 Skill，见 `.cursor/README.md` 表 | `.cursor/skills/xgboost-studio-role-*/SKILL.md` |
...
**五领域分角目录名**（审视用；签核表旧列名映射见 `.cursor/README.md` § Subagents）：`xgboost-studio-role-data-reproducibility`、`xgboost-studio-role-modeling-eval`、`xgboost-studio-role-product-experience`、`xgboost-studio-role-systems-contract`、`xgboost-studio-role-quality-gate`（测试落地仍用 `xgboost-studio-test-delivery`）。
