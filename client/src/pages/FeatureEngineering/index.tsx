...
import {
  ...
  Checkbox, Slider, InputNumber, message, Alert, Tag, Form, Input, Divider
} from 'antd'
import { ToolOutlined, FilterOutlined, CompressOutlined } from '@ant-design/icons'
              ...
              <Card style={{ background: '#1e293b', border: '1px solid #334155' }}>
                <Form layout="vertical">
                  <Form.Item label={<Text style={{ color: '#94a3b8' }}>处理策略</Text>}>
                    <Select value={missingStrategy} onChange={setMissingStrategy} style={{ width: 200 }}
                      options={[
                        { value: 'mean', label: '均值填充' }, { value: 'median', label: '中位数填充' },
                        { value: 'mode', label: '众数填充' }, { value: 'constant', label: '固定值填充(0)' },
                        { value: 'drop', label: '删除含缺失的行' }
                      ]} />
                  </Form.Item>
                  <Form.Item label={<Text style={{ color: '#94a3b8' }}>指定列（留空表示全部数值列，逗号分隔）</Text>}>
                    <Input value={missingCols} onChange={e => setMissingCols(e.target.value)} style={{ background: '#0f172a', borderColor: '#334155', color: '#e2e8f0' }} placeholder="col1,col2,..." />
                  </Form.Item>
                  <Button type="primary" loading={loading}
                    onClick={() => exec('missing', { strategy: missingStrategy, columns: missingCols ? missingCols.split(',').map(s => s.trim()) : null })}>
                    执行缺失值处理
                  </Button>
                </Form>
              </Card>
              ...
              <Card style={{ background: '#1e293b', border: '1px solid #334155' }}>
                <Form layout="vertical">
                  <Form.Item label={<Text style={{ color: '#94a3b8' }}>处理方式</Text>}>
                    <Select value={outlierStrategy} onChange={setOutlierStrategy} style={{ width: 200 }}
                      options={[
                        { value: 'clip', label: 'IQR截断（上下限）' },
              ...
              <Card style={{ background: '#1e293b', border: '1px solid #334155' }}>
                <Form layout="vertical">
                  <Form.Item label={<Text style={{ color: '#94a3b8' }}>编码方式</Text>}>
                    <Select value={encodeMethod} onChange={setEncodeMethod} style={{ width: 200 }}
                      options={[
                  ...
                  <Form.Item label={<Text style={{ color: '#94a3b8' }}>目标列（逗号分隔）</Text>}>
                    <Input value={encodeCols} onChange={e => setEncodeCols(e.target.value)} style={{ background: '#0f172a', borderColor: '#334155', color: '#e2e8f0' }} placeholder="col1,col2,..." />
                  </Form.Item>
                  <Button type="primary" loading={loading}
                    onClick={() => exec('encode', { method: encodeMethod, columns: encodeCols.split(',').map(s => s.trim()).filter(Boolean) })}>
                    执行编码
                  </Button>
                </Form>
              </Card>
              ...
              <Card style={{ background: '#1e293b', border: '1px solid #334155' }}>
                <Form layout="vertical">
                  <Form.Item label={<Text style={{ color: '#94a3b8' }}>缩放方式</Text>}>
                    <Select value={scaleMethod} onChange={setScaleMethod} style={{ width: 200 }}
                      options={[
                  ...
                  <Form.Item label={<Text style={{ color: '#94a3b8' }}>目标列（留空=全部数值列，逗号分隔）</Text>}>
                    <Input value={scaleCols} onChange={e => setScaleCols(e.target.value)} style={{ background: '#0f172a', borderColor: '#334155', color: '#e2e8f0' }} placeholder="col1,col2,..." />
                  </Form.Item>
                  <Button type="primary" loading={loading}
                    onClick={() => exec('scale', { method: scaleMethod, columns: scaleCols ? scaleCols.split(',').map(s => s.trim()).filter(Boolean) : null })}>
                    执行缩放
                  </Button>
                </Form>
              </Card>
              ...
              <Card style={{ background: '#1e293b', border: '1px solid #334155' }}>
                <Form layout="vertical">
                  <Form.Item label={<Text style={{ color: '#94a3b8' }}>目标列</Text>}>
                    <Input value={targetCol} onChange={e => setTargetCol(e.target.value)} style={{ background: '#0f172a', borderColor: '#334155', color: '#e2e8f0' }} placeholder="目标列列名" />
                  </Form.Item>
                  ...
                  {splitResult && (
                    <Alert
                      style={{ marginTop: 16 }}
                      type="success"
                      message={`划分成功 | Split ID: ${splitResult.split_id} | 训练: ${splitResult.train_rows} 行 | 测试: ${splitResult.test_rows} 行`}
                    />
                  )}
                </Form>
              </Card>
