...
| 项目 | 说明 |
|------|------|
| 架构 | 与 CI（`macos-latest`）一致，以资产文件名中的 **`-arm64` / `-x64`** 为准。 |
...
| 平台 | 行为 |
|------|------|
| **macOS** | PyInstaller → `client/resources/xgboost-server` → `npm ci` + `npm run build` → 仓库根 `dist/` 下 dmg/zip。 |
...
| Release 典型资产 | NSIS + portable `.exe` | `.dmg` + `.zip` | `xgboost-server-*-linux-x64.tar.gz` |
| Electron | 有 | 有 | **无** |
